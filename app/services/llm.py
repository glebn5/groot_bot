...
{{
  ...
  "repeat_type": "daily" | "weekly" | "interval_days" | "custom_cron" (тип повторения),
  "repeat_days": ["mon", "tue", "wed", "thu", "fri", "sat", "sun"] (массив дней для weekly повторений),
  "repeat_interval": number (интервал в днях для interval_days, например 4 для 'каждые 4 дня'),
...
13. Повторяющиеся задачи и привычки ("Напоминай каждый день...", "Каждый вторник и пятницу...", "Каждые 4 дня...", "По будням...", "Мои привычки"):
   - При добавлении ("Напоминай пить воду каждый день в 11:00"): `is_recurring_add`: true, `recurring_title`: "Пить воду", `repeat_type`: "daily", `repeat_time`: "11:00".
   ...
   - Для правил с интервалом ("Каждые 4 дня менять фильтр в 10:00"): `is_recurring_add`: true, `recurring_title`: "Менять фильтр", `repeat_type`: "interval_days", `repeat_interval`: 4, `repeat_time`: "10:00".
   ...
   - Для выходных ("По выходным в 10:00 уборка"): `is_recurring_add`: true, `recurring_title`: "Уборка", `repeat_type`: "weekly", `repeat_days`: ["sat", "sun"], `repeat_time`: "10:00".
