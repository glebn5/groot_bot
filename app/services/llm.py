...
{{
  ...
  "is_goal_add": boolean (true если пользователь просит поставить/добавить цель на месяц),
  "goal_text": "Текст цели (например: 'закрыть проект', 'прочитать 2 книги')",
  "target_month": "YYYY-MM" (месяц цели в формате YYYY-MM, например '2026-09' или '2026-10', по умолчанию текущий месяц),
  "is_goals_query": boolean (true если пользователь спрашивает цели на месяц, 'мои цели', 'покажи цели'),
  "is_recurring_add": boolean (true если пользователь просит создать повторяющуюся задачу или привычку),
  "recurring_title": "Текст привычки/повторяющейся задачи (например: 'пить воду', 'поливать цветы')",
  "repeat_type": "daily" | "weekly" | "interval_days" | "interval_hours" | "interval_minutes" | "custom_cron" (тип повторения),
  "repeat_days": ["mon", "tue", "wed", "thu", "fri", "sat", "sun"] (массив дней для weekly повторений),
  "repeat_interval": number (интервал для interval_days/hours/minutes, например 1 для 'каждый час', 2 для 'каждые 2 часа', 4 для 'каждые 4 дня', 30 для 'каждые 30 минут'),
  "repeat_time": "HH:MM" (время суток, например '11:00' или '18:00'),
  "is_recurring_query": boolean (true если пользователь просит показать привычки/повторяющиеся задачи),
...
12. Квалификация целей на месяц ("Поставь цель на этот месяц...", "Цель на октябрь...", "Мои цели на месяц"):
   - При добавлении цели ("Поставь цель на этот месяц закрыть проект", "Цель на октябрь: прочитать 2 книги"): установи `is_goal_add`: true, `goal_text`: текст цели ("закрыть проект", "прочитать 2 книги"), `target_month`: месяц цели в формате YYYY-MM (например, '2026-09' или '2026-10'). Если месяц не указан явно, используй текущий месяц из {current_datetime}.
   - При запросе целей ("покажи цели", "какие цели на этот месяц", "мои цели"): установи `is_goals_query`: true.
13. Повторяющиеся задачи и привычки ("Напоминай каждый день...", "Каждый вторник и пятницу...", "Каждый час пить воду", "Каждые 2 часа...", "Каждые 4 дня...", "По будням...", "Мои привычки"):
   - Для повтора каждый час / каждые N часов ("напоминай мне каждый день каждый час пить воду", "каждый час пить воду"): `is_recurring_add`: true, `recurring_title`: "Пить воду", `repeat_type`: "interval_hours", `repeat_interval`: 1.
   - Для каждые N часов ("каждые 2 часа разминаться"): `is_recurring_add`: true, `recurring_title`: "Разминаться", `repeat_type`: "interval_hours", `repeat_interval`: 2.
   - Для каждые N минут ("каждые 30 минут пить воду"): `is_recurring_add`: true, `recurring_title`: "Пить воду", `repeat_type`: "interval_minutes", `repeat_interval`: 30.
   - При добавлении ежедневной ("Напоминай пить воду каждый день в 11:00"): `is_recurring_add`: true, `recurring_title`: "Пить воду", `repeat_type`: "daily", `repeat_time`: "11:00".
   - Для конкретных дней недели ("Каждый вторник и пятницу в 18:00 поливать цветы"): `is_recurring_add`: true, `recurring_title`: "Поливать цветы", `repeat_type`: "weekly", `repeat_days`: ["tue", "fri"], `repeat_time`: "18:00".
   - Для правил с интервалом в днях ("Каждые 4 дня менять фильтр в 10:00"): `is_recurring_add`: true, `recurring_title`: "Менять фильтр", `repeat_type`: "interval_days", `repeat_interval`: 4, `repeat_time`: "10:00".
   - Для будней ("По будням в 9:00 зарядка"): `is_recurring_add`: true, `recurring_title`: "Зарядка", `repeat_type`: "weekly", `repeat_days`: ["mon", "tue", "wed", "thu", "fri"], `repeat_time`: "09:00".
   - При запросе списка привычек ("мои привычки", "повторяющиеся задачи", "покажи привычки"): `is_recurring_query`: true.
14. Правило 12-часового формата времени для текущего дня ("сегодня в 8", "в 7", "сегодня в 9"):
...
15. Ответ ДОЛЖЕН БЫТЬ только чистым валидным JSON без разметки markdown и без комментов.
16. В confirmation_text сформируй короткое, дружелюбное и тёплое подтверждение пользователю в стиле персонажа Грута (начинай с эмодзи 🌴 или ✨, например: "🌴 Отлично, всё запомнил!", "🌴 Готово! Задачи добавил!"), без упоминания названий внешних сервисов (Obsidian/Календарь).
"""
